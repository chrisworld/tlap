...
sub luminance_change {
  ...
  for ( my $i = 0; $i < $max_entries; $i++ ) {
    ...
    my $deflicker_dir = $data_dir."/deflickered";
    if ( !-d $deflicker_dir ) {
      mkdir($deflicker_dir) || die "Error creating directory: $!\n";
    }
    ...
    $image->Write( $deflicker_dir."/" . $fname{$i}{filename} );
...
sub usage {
  ...
  say "-v    Verbose";
  say "-d    Debug";
  say "-P    Path to timelapse directory";
}
